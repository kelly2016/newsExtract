...
Demo.prototype = {
  ...
  processSrlNode : function (idx, arg, drawStruct) {
    ...
    // update for SDP with Multi-Parents
    processSdpGraphNode : function(curIdx, sem, drawStruct) {
        if(sem == undefined || sem.length == 0){
            console.error("At idx: " + curIdx + ", no semantic info!");
            drawStruct.sdpGraph.push([]);
            return;
        }
        var curSdpNode = [];
        for(var i = 0; i < sem.length; ++i){
            var oneSdpArc = {};
            oneSdpArc["from"] = parseInt(sem[i].parent);
            oneSdpArc["to"] = parseInt(curIdx);
            oneSdpArc["relate"] = sem[i].relate;
            curSdpNode.push(oneSdpArc);
        }
        drawStruct.sdpGraph.push(curSdpNode);
    },
      ...
      DP_DISCONF : {
        ...
        "textColor" : "red"
            ...
            ALL_ELEMENTS_HIGHT_CONF : {
                ...
                'POSTAG' : 50 , 
                'NER' : 50 ,
                'SRL' : 1000 , // to make it big enough when initialization .
                'DP' : 5000 ,
                'SDP' : 5000,
                'SDP_GRAPH': 5000
            }
    ...
    for (i = 0; i < sentObj.length; i++) {
      ...
      this.drawStruct.dp.push({
        ...
        "relate" : sentObj[i].relate
            ...
            // for obj.name -> if name not in obj, an TypeError report.
            // for obj["name"] -> if name not in obj, undefined retured.
            this.processSdpGraphNode(i, sentObj[i]["sem"], this.drawStruct);
    }
  ...
  drawDP : function (drawStruct) {
    ...
    drawSdpGraphNode : function(sdpGraphNode, drawStruct, cxt){
        if(sdpGraphNode.length == 0){
            return 0;
        }
        var oneSdpArc,
            fromCenter,
            ...
            ;
        // draw every line
        for(var i = 0; i < sdpGraphNode.length; ++i){
            oneSdpArc = sdpGraphNode[i];
            fromCenter = ( 
                            drawStruct.posInfo[oneSdpArc.from + 1] + drawStruct.posInfo[oneSdpArc.from + 1 + 1] - 
                            drawStruct.WS_INTERVAL 
                        ) / 2;
            toCenter = ( 
                         drawStruct.posInfo[oneSdpArc.to + 1] + drawStruct.posInfo[oneSdpArc.to + 1 + 1] - 
                         drawStruct.WS_INTERVAL
                       ) / 2;
            fromX = fromCenter < toCenter ? fromCenter + lineInterval : fromCenter - lineInterval;
            controlPointHeight = Math.abs(oneSdpArc.to - oneSdpArc.from) * 13 + 10;
            controlPointY = lineY - controlPointHeight;
            actualY = lineY - controlPointHeight * 3 / 4;
            cxt.strokeStyle = drawStruct.SDP_GRAPH_DISCONF.lineColor;
            cxt.beginPath(); // !!!! must clear previous path ! or previous path will be repaint again and again , then the line become ugly with jaggies
            cxt.moveTo(fromX, lineY);
            cxt.bezierCurveTo(fromX, controlPointY, toCenter, controlPointY, toCenter, lineY);
            cxt.stroke();
            cxt.closePath();
            //draw relate text
            cxt.fillStyle = drawStruct.SDP_GRAPH_DISCONF.textColor ;
            cxt.textAlign = "center";
            this.setEnTextFont(cxt);
            cxt.fillText(oneSdpArc.relate, (fromX + toCenter) / 2, actualY + this.getFontSizeFromFontstr(cxt.font) / 2);
        }
        // draw arrow
        // - if there are more than one arc, we just draw the vertical arrow
        // - else we draw a skewed arrow (skew to `from` node)
        arrowLeftOffset = sdpGraphNode.length > 1 ? (arrowWidth - cxt.lineWidth) / 2 
                                             : sdpGraphNode[0].to > sdpGraphNode[0].from ? (arrowWidth - cxt.lineWidth) / 2 + 1 
                                                                               : (arrowWidth - cxt.lineWidth) / 2 - 1;
        cxt.beginPath();
        cxt.moveTo(toCenter, paintY);
        cxt.lineTo(toCenter - arrowLeftOffset, paintY - arrowHeight);
        cxt.lineTo(toCenter + (arrowWidth - arrowLeftOffset), paintY - arrowHeight);
        cxt.closePath();
        cxt.fillStyle = drawStruct.SDP_GRAPH_DISCONF.lineColor ;
        cxt.fill();
        return actualY;
    },
    drawSDPGraph : function(drawStruct){
        if(drawStruct.sdpGraph.length == 0) return null ;
    var sdpGraphCanvas = this.createCanvasBuffer(drawStruct.width, drawStruct.ALL_ELEMENTS_HIGHT_CONF['SDP_GRAPH']),
        wsCanvas = null ,
        wholeCanvas = null ,
    cxt = sdpGraphCanvas.getContext("2d"),
    i,
    minY = sdpGraphCanvas.height,
    newHeight ,
        wholeWidth ,
        wholeHeight;
    cxt.lineWidth = 0.6;
    for (i = 0; i < drawStruct.sdpGraph.length; i++) {
      var yPos = this.drawSdpGraphNode(drawStruct.sdpGraph[i], drawStruct, cxt);
      if (minY > yPos)
        minY = yPos;
    }
    minY -= 10; //  the up margin
    //move the image to the right position
    //first we should save the image data , then change the canvas height and put the imageData to it
    newHeight = sdpGraphCanvas.height - minY;
    this.setCanvasAppropriateHeight(sdpGraphCanvas, minY, newHeight);
        drawStruct.ALL_ELEMENTS_HIGHT_CONF['SDP_GRAPH'] = newHeight ; 
    
        // here we should add the ws result under the sdp-graph lines
        wsCanvas = this.drawWS(drawStruct) ;
        wholeWidth = Math.max(sdpGraphCanvas.width , wsCanvas.width) ;
        wholeHeight = sdpGraphCanvas.height + wsCanvas.height ;
        wholeCanvas = this.createCanvasBuffer(wholeWidth , wholeHeight) ;
        cxt = wholeCanvas.getContext('2d') ;
        cxt.drawImage(sdpGraphCanvas , 0 , 0) ;
        cxt.drawImage(wsCanvas , 0 , sdpGraphCanvas.height) ;
        return wholeCanvas ;
    } ,
  /*
   **according to the drawStruct , paint the six components
   * return value : a object contains the six components
   */
  drawComponent : function (drawStruct) {
    ...
    DPCanvas = this.drawDP(drawStruct);
        ...
        // draw SDPGraph
        SDPGraphCanvas = this.drawSDPGraph(drawStruct);
    ...
    this.canvasBufs["DP"] = DPCanvas;
        ...
        this.canvasBufs["SDP_GRAPH"] = SDPGraphCanvas;
  ...
  drawContainer : function (drawStruct, canvasBufs, disableAttr) {
    ...
    //bulid the drawBufs
    if (typeof disableAttr == "undefined" || disableAttr == null){ disableAttr = {}; }
    ...
    drawTarget = ["DP", "WS", "POSTAG", "NER", "SRL" , "SDP", "SDP_GRAPH"];
